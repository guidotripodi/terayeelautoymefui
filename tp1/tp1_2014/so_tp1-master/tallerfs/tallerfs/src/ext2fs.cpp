...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number) {
	...
	Ext2FSBlockGroupDescriptor * blockgroup = block_group(blockgroup_index);

	return blockgroup->inode_table + blockgroup_inode_index(inode_number);
}

unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int block_table_size = block_size / sizeof(unsigned int);

	if (block_number < 12) {
		return inode->block[block_number];
	} else if (block_number < block_table_size + 12) {
		unsigned char * buffer[block_table_size];
		read_block(inode->block[12], buffer);
		block_number = block_number -12;
		return buffer[block_number];
	} else {
		block_table_size = block_table_size +12;
		unsigned char * buffer[block_table_size];
		read_block(inode->block[13], buffer);
		unsigned int contador_tabla_indirecta = block_table_size;
		for (int i = 1; i < block_number; ++i) {
			if (block_number <  contador_tabla_indirecta + (block_table_size * i)){
				unsigned char * buffer[block_table_size];
				read_block(inode->block[contador_tabla_indirecta + (block_table_size * i)], buffer);
				block_number = block_number - (contador_tabla_indirecta + (block_table_size * (i-1)));
				return buffer[block_number];
			}
